...
extern "C"
{
	__declspec(selectany) __declspec(allocate(ARXCOMMAND_FIRST_ENTRYNAME))
			_ARXCOMMAND_ENTRY* __pArxCmdMapEntryFirst = NULL;
	__declspec(selectany) __declspec(allocate(ARXCOMMAND_LAST_ENTRYNAME))
...
#define ACED_ARXCOMMAND_ENTRY_AUTO(classname, group, globCmd, locCmd, cmdFlags, UIContext)    \
	__declspec(selectany)                                                                     \
			_ARXCOMMAND_ENTRY __ArxCmdMap_##group##globCmd = {_RXST(#group),                  \
															  _RXST(#globCmd),                \
															  _RXST(#locCmd),                 \
															  cmdFlags,                       \
															  classname::group##globCmd,      \
															  UIContext,                      \
															  0};                             \
	extern "C" __declspec(allocate("ARXCOMMAND$__m")) __declspec(selectany)                   \
			_ARXCOMMAND_ENTRY* __pArxCmdMap_##group##globCmd = &__ArxCmdMap_##group##globCmd; \
...
#define ACED_ARXCOMMAND_ENTRYBYID_AUTO(classname, group, globCmd, locCmdId, cmdFlags, UIContext) \
	__declspec(selectany) _ARXCOMMAND_ENTRY __ArxCmdMap_##group##globCmd = {                     \
			_RXST(#group), _RXST(#globCmd), NULL, cmdFlags, classname::group##globCmd,           \
			UIContext,	   locCmdId};                                                            \
	extern "C" __declspec(allocate("ARXCOMMAND$__m")) __declspec(selectany)                      \
			_ARXCOMMAND_ENTRY* __pArxCmdMap_##group##globCmd = &__ArxCmdMap_##group##globCmd;    \
	ACED_ARXCOMMAND_ENTRY_PRAGMA(group, globCmd)
...
extern "C"
{
	__declspec(selectany) __declspec(allocate(ADSSYMBOL_FIRST_ENTRYNAME))
			_ADSSYMBOL_ENTRY* __pAdsSymbolMapEntryFirst = NULL;
	__declspec(selectany) __declspec(allocate(ADSSYMBOL_LAST_ENTRYNAME))
			_ADSSYMBOL_ENTRY* __pAdsSymbolMapEntryLast = NULL;
}

#define ACED_ADSSYMBOL_ENTRY_PRAGMA(name) \
	__pragma(comment(linker, "/include:__pAdsSymbolMap_" #name));

#define ACED_ADSSYMBOL_ENTRY_AUTO(classname, name, regFunc)                    \
	__declspec(selectany) _ADSSYMBOL_ENTRY __AdsSymbolMap_##name = {           \
			_RXST(#name), classname::ads_##name, regFunc, -1};                 \
	extern "C" __declspec(allocate("ADSSYMBOL$__m")) __declspec(selectany)     \
			_ADSSYMBOL_ENTRY* __pAdsSymbolMap_##name = &__AdsSymbolMap_##name; \
	ACED_ADSSYMBOL_ENTRY_PRAGMA(name)
#define ACED_ADSCOMMAND_ENTRY_AUTO(classname, name, regFunc)                   \
	__declspec(selectany) _ADSSYMBOL_ENTRY __AdsSymbolMap_##name = {           \
			_RXST("c:") _RXST(#name), classname::ads_##name, regFunc, -1};     \
	extern "C" __declspec(allocate("ADSSYMBOL$__m")) __declspec(selectany)     \
			_ADSSYMBOL_ENTRY* __pAdsSymbolMap_##name = &__AdsSymbolMap_##name; \
	ACED_ADSSYMBOL_ENTRY_PRAGMA(name)

#define ACED_ADSSYMBOL_ENTRYBYID_AUTO(classname, name, nameId, regFunc)                          \
	__declspec(selectany) _ADSSYMBOL_ENTRY __AdsSymbolMap_##name = {NULL, classname::ads_##name, \
																	regFunc, nameId};            \
	extern "C" __declspec(allocate("ADSSYMBOL$__m")) __declspec(selectany)                       \
			_ADSSYMBOL_ENTRY* __pAdsSymbolMap_##name = &__AdsSymbolMap_##name;                   \
	ACED_ADSSYMBOL_ENTRY_PRAGMA(name)
...
public:
	...
	virtual AcRx::AppRetCode On_kInitAppMsg(void* pkt)
	{
		...
		ACHAR buffer[133];
		for (_ARXCOMMAND_ENTRY** ppEntry = ppArxCmdMapEntryFirst; ppEntry < ppArxCmdMapEntryLast;
			 ppEntry++)
		{
			if (*ppEntry != NULL)
			{
				...
				acedRegCmds->addCommand(
						(*ppEntry)->pszCmdGroupName, (*ppEntry)->pszCmdGlobalName,
						(*ppEntry)->pszCmdLocalName == NULL ? buffer : (*ppEntry)->pszCmdLocalName,
						(*ppEntry)->commandFlags, (*ppEntry)->pCmdFct, (*ppEntry)->pUIContext, -1,
	...
	virtual AcRx::AppRetCode On_kInvkSubrMsg(void* pkt)
	{
		...
		for (_ADSSYMBOL_ENTRY** ppEntry = ppAdsSymbolMapEntryFirst;
			 ppEntry < ppAdsSymbolMapEntryLast; ppEntry++)
		{
			if (*ppEntry != NULL)
			{
				if (paramIter++ == param)
