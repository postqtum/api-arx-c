...
extern "C"
{
	__declspec(selectany) __declspec(allocate(DBXCUSTOBJ_FIRST_ENTRYNAME))
			_DBX_REGISTER_OBJECT_ENTRY* __pDbxCustObjMapEntryFirst = NULL;
	__declspec(selectany) __declspec(allocate(DBXCUSTOBJ_LAST_ENTRYNAME))
			_DBX_REGISTER_OBJECT_ENTRY* __pDbxCustObjMapEntryLast = NULL;
}

#define ACDB_REGISTER_OBJECT_ENTRY_PRAGMA(classname) \
	__pragma(comment(linker, "/include:__pDbxCustObjMap_" #classname));

#define ACDB_REGISTER_OBJECT_ENTRY_AUTO(classname)                                   \
	__declspec(selectany) _DBX_REGISTER_OBJECT_ENTRY __DbxCustObjMap_##classname = { \
			classname::desc, classname::rxInit};                                     \
	extern "C" __declspec(allocate("DBXCUSTOBJ$__m")) __declspec(selectany)          \
			_DBX_REGISTER_OBJECT_ENTRY* __pDbxCustObjMap_##classname =               \
					&__DbxCustObjMap_##classname;                                    \
	ACDB_REGISTER_OBJECT_ENTRY_PRAGMA(classname)

#define DISPATCH_PKTMSG(pktmsg) \
	case AcRx::pktmsg: return (On_##pktmsg(pkt))
...
private:
	void DeleteClassAndDescendant(_DBX_REGISTER_OBJECT_ENTRY** ppEntry)
	{
		...
		for (_DBX_REGISTER_OBJECT_ENTRY** ppEntry2 = ppDbxCustObjMapEntryFirst;
			 ppEntry2 < ppDbxCustObjMapEntryLast; ppEntry2++)
		{
...
public:
	...
	virtual AcRx::AppRetCode On_kInitAppMsg(void* pkt)
	{
		...
		_DBX_REGISTER_OBJECT_ENTRY** ppDbxCustObjMapEntryFirst = &__pDbxCustObjMapEntryFirst + 1;
		_DBX_REGISTER_OBJECT_ENTRY** ppDbxCustObjMapEntryLast = &__pDbxCustObjMapEntryLast;
		for (_DBX_REGISTER_OBJECT_ENTRY** ppEntry = ppDbxCustObjMapEntryFirst;
			 ppEntry < ppDbxCustObjMapEntryLast; ppEntry++)
		{
			if (*ppEntry != NULL)
			{
				nDbxCustObjCount++;
